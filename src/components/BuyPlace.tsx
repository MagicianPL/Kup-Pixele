...
const BuyPlace = () => {
  ...
  const nonLimitedPlaces = places.filter(
    (place: any) => place.isLimited === false && place.isSold === false
  );
  ...
  useEffect(() => {
    if (
      inputValues.background !== "#21092f" &&
      inputValues.background !== "#a62508" &&
      inputValues.background !== "#0da608"
    ) {
      setBorderOn(null);
    }
  }, [inputValues]);
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isValid) {
      ...
      if (
        inputValues.url.indexOf("http://") !== 0 &&
        inputValues.url.indexOf("https://") !== 0
      ) {
        setInputValues({ ...inputValues, url: `https://${inputValues.url}` });
      }
      setShowLoadingModal(true);
      const res = await fetch(
        "https://busy-gray-prawn-hem.cyclic.app/api/pixels/buy/nonlimited",
        {
          method: "POST",
          headers: {
  ...
  return (
    <StyledWrapper>
      <CenteredContainer>
        ...
        <form onSubmit={handleFormSubmit}>
          ...
          <div className="colors">
            <div
              className={borderOn === "violet" ? "border" : ""}
              data-value="violet"
              onClick={handleColorClick}
            ></div>
            <div
              className={borderOn === "red" ? "border" : ""}
              data-value="red"
              onClick={handleColorClick}
            ></div>
            <div
              className={borderOn === "green" ? "border" : ""}
              data-value="green"
              onClick={handleColorClick}
            ></div>
          </div>
          ...
          <Input
            ...
            label={`Ilość miejsc ${
              places.length > 0
                ? `(${nonLimitedPlaces.length} dostępnych z edycji nielimitowanej)`
                : ""
            }`}
      ...
      {showLoadingModal && (
        <BuyingPlaceInfoModal fetchingError={fetchingError} />
      )}
    </StyledWrapper>
  );
};
